...
- *Expiry* = max(`passedat` + merged `validitymonths`, `extendeduntil`), recomputed on every read so profile changes apply; the stored `validuntil` is informational. **No expiry at all when validity is 0**, extension or not: never-expires ignores extensions (`extendcertification` refuses such a topic with `never_expires`).
- *Window open* = expiry set AND the expiry day is at most `renewalwindowdays` calendar days away in the org timezone (`DAYS.between(today, expiryday) <= renewalwindowdays`), so the status flips at the start of the window day and the `window_open` reminder fires on the same day; `windowopens` = expiry day − `renewalwindowdays`.
...
- **canstart** (replaces "`passed` is terminal"): for a certification topic, an evaluation may start when status ∈ {`not_certified`, `renewal_due`, `expired`}; `certified` reports `evaluation_not_available {status: certified, reason: valid_until, validuntil}`. Retake wait and `maxattempts` still apply; attempts are counted **per cycle** (finalized attempts with `submitted` after max(`passedat`, expiry) -- after `passedat` while the certificate is valid, and after the expiry once it has lapsed: an expired certification starts a fresh attempt budget, so a learner who burned every renewal attempt inside the window may sit it again once it expires; all attempts count when there is no pass). Non-certification topics keep today's rule unchanged.
...
- `learn/extendcertification.json` (POST `user`, `topicid`, `until` = `YYYY-MM-DD`, `reason`) → `{ok, row}`. Requires a row with `passedat`; `until` > current expiry and ≤ today + 12 months; `reason` non-blank. Writes `extendeduntil`, `extendreason`, `extendedby`; clears the `7d` / `1d` / `expired` stages from `reminderssent` so they fire again for the new date. 400 `not_certification_topic` | `never_expires` (`validitymonths` = 0: there is no expiry to push out, and `expiryOf` ignores `extendeduntil` for it) | `not_certified_yet` | `bad_date` | `date_not_later` | `date_too_far` | `missing_reason`; 403 `forbidden` (checked before the user lookup, so a learner cannot probe who exists); 404 `unknown_user` | `unknown_topic`. Audited `certification.extend`.
...
- `permissionentityassigned` rows for `manageevaluations` (per-topic trainer scope) must exist on the target server before trainers use the Personas tab; this includes an explicit grant for administrators (rows do not auto-load on an existing table; the local DB needed an explicit administrators grant for topics to appear).
  Concretely, per server (from the 2026-09-23 review):
  - Full admins: one module-level row `{moduleid: entitytopic, entityid: <none>, permissionsentity: manageevaluations, group: administrators, enabled: true}`.
  - Scoped trainer groups: **no** module-level row, and one row per owned topic `{entityid: <topicid>, permissionsentity: manageevaluations, group: <group>}` -- a module-level row defeats per-topic scoping, so it must exist only for the groups meant to see every topic.
  - The `permissionsentity` row `manageevaluations` itself (`plugins/catalog/html/data/lists/permissionsentity/evaluations.xml`) must be loaded into the existing table (PUT it, or reindex) -- an existing table does not pick up the file on its own.
  - Restart or clear the permissions cache after inserting rows; they are cached per profile.
  - `enabled` on `permissionentityassigned` rows is ignored by `calculateEntityPermissions`: the presence of the row grants it, so revoking means deleting the row.
  - `certifications.json` replies `hidden: n`, the number of certification topics dropped by this check, and the Personas tab shows "N temas ocultos por tus permisos" so a missing grant is visible instead of looking like an empty roster.
