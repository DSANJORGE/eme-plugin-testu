...
PY
code=$(curl -s -o /dev/null -w '%{http_code}' -b "$J" "$B/services/testu/analytics/person.json?user=nobody"); [ "$code" = 403 ] && echo "ok: out-of-scope person is 403" || { echo "FAIL: $code"; SAID=1; exit 1; }
